...
class PokedexHomeViewModel @Inject constructor(
    ...
    private suspend fun insertDummyPokemonData(){
        ...
        val pokemonData = pokemonRawData.map { rawPokemon ->
            ...
            val urlRawPokeData = rawPokeData[2].split(":")
            val urlPokeData = urlRawPokeData[1] + ":" + urlRawPokeData[2]
            com.example.database.Pokemon(
                id = pokemonRawData.indexOf(element = rawPokemon) + 1,
                name = namePokeData.substring(1, namePokeData.length - 1),
                url = urlPokeData.substring(1, urlPokeData.length - 1)
            )
        }
    ...
    private suspend fun getPokemon(){
        pokemonRepository.getAllPokemon().collect { allPokemon ->
            ...
            Log.d("Pokemon", "Pokemon: $allPokemon")
        }
    }
    ...
    override fun pokemonClicked(pokemon: com.example.database.Pokemon) {
